...
def etl(
    ...
    while True:
        person_ids, persons = pg_extract.modified_persons(bach_size)
        genre_ids, genres = pg_extract.modified_genres(bach_size)
        filmwork_ids = pg_extract.modified_filmwork(bach_size)
        modified_filmworks_ids = set(filmwork_ids + person_ids + genre_ids)

        if (len(modified_filmworks_ids) or len(persons + genres)) != 0:
            data = pg_extract.enrich_filmwork_by_id(modified_filmworks_ids)

            response = pg_extract.transform_data(data) + persons + genres
